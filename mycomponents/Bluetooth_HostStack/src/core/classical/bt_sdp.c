...
static uint8_t sdp_de_read_seq_len(const uint8_t *de, uint16_t *hdr_len, uint16_t *value_len)
{
    uint8_t sz_type;

    if((de == NULL) || (hdr_len == NULL) || (value_len == NULL))
    {
        return 0;
    }

    if(SDP_DE_TYPE(de[0]) != SDP_DE_TYPE_DES)
    {
        return 0;
    }

    sz_type = SDP_DE_SIZE(de[0]);
    if(sz_type == SDP_DE_SIZE_N1)
    {
        *hdr_len = 2u;
        *value_len = de[1];
        return 1;
    }
    if(sz_type == SDP_DE_SIZE_N2)
    {
        *hdr_len = 3u;
        *value_len = bt_be_read_16(de, 1);
        return 1;
    }
    if(sz_type == SDP_DE_SIZE_N4)
    {
        uint32_t vlen = bt_be_read_32(de, 1);
        if(vlen > 0xFFFFu)
        {
            return 0;
        }
        *hdr_len = 5u;
        *value_len = (uint16_t)vlen;
        return 1;
    }
...
uint8_t sdp_pattern_search(sdp_record_t *record, uint16_t size, struct bt_pbuf_t *p)
{
    uint16_t i, j;
    ...
    for(i = 0; i < size; ++i)
    {
        if(SDP_DE_TYPE(payload[i]) == SDP_DE_TYPE_UUID)     //判断是否是UUID，UUID有16/32/128位三种：16位直接比较，32位跳过，128位转换后比较
        {
...
struct bt_pbuf_t *sdp_attribute_search(uint16_t max_attribl_bc, struct bt_pbuf_t *p, sdp_record_t *record)
{
    ...
    if(sdp_de_read_seq_len(payload, &de_hdr_len, &size))
    {
        while(i < size)
        {
            uint16_t off = (uint16_t)(de_hdr_len + i);
            if(off >= (uint16_t)(de_hdr_len + size))
            {
                break;
            }
            /* Check if this is an attribute ID or a range of attribute IDs */
            if(payload[off] == (SDP_DE_TYPE_UINT  | SDP_DE_SIZE_16))
            {
                if((off + 2u) >= (uint16_t)(de_hdr_len + size))
                {
                    break;
                }
            	attr_id = *(payload + off + 1) << 8 | *(payload + off + 2);
                attr_id2 = attr_id; /* For the range to cover this attribute ID only */
                i = (uint16_t)(i + 3u);
            }
            else if(payload[off] == (SDP_DE_TYPE_UINT | SDP_DE_SIZE_32))
            {
                if((off + 4u) >= (uint16_t)(de_hdr_len + size))
                {
                    break;
                }
            	/* attr range is big-endian */
				attr_id = *(payload + off + 1) << 8 | *(payload + off + 2);
                attr_id2 = *(payload + off + 3) << 8 | *(payload + off + 4);
                i = (uint16_t)(i + 5u);
            }
            else
            {
...
err_t sdp_service_search_rsp(l2cap_pcb_t *pcb, struct bt_pbuf_t *p, sdp_hdr_t *reqhdr)
{
    ...
    //检查格式
    if(sdp_de_read_seq_len((uint8_t *)p->payload, &de_hdr_len, &size))
    {
        /* Get maximum service record count that follows the service search pattern */
        max_src = bt_ntohs(*((uint16_t *)(((uint8_t *)p->payload)+(de_hdr_len+size))));

        bt_pbuf_header(p, -(int16_t)de_hdr_len);
    }
    else
    {
...
err_t sdp_service_search_attrib_rsp(l2cap_pcb_t *pcb, struct bt_pbuf_t *p, sdp_hdr_t *reqhdr)
{
    ...
    if(sdp_de_read_seq_len((uint8_t *)p->payload, &de_hdr_len, &size))
    {
        ...
        max_attribl_bc = bt_ntohs(*((uint16_t *)(((uint8_t *)p->payload)+(de_hdr_len+size))));
    ...
    for(record = sdp_server_records; record != NULL; record = record->next)
    {
        ...
        {
            /* Search for attributes and add them to a bt_pbuf_t */
            bt_pbuf_header(p, -(int16_t)(size + 2u));
            r = sdp_attribute_search(max_attribl_bc, p, record);
...
err_t sdp_recv(void *arg, l2cap_pcb_t *pcb, struct bt_pbuf_t *s, err_t err)
{
    ...
    if(s->len != s->tot_len)    //多buf数据
    {
        BT_SDP_TRACE_DEBUG("sdp_recv: Fragmented packet received. Reassemble into one buffer\n");
        if((p = bt_pbuf_alloc(BT_PBUF_RAW, s->tot_len, BT_PBUF_RAM)) != NULL)       //拷贝数据到P
        {
            i = 0;
