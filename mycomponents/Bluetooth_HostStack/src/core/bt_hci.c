...
err_t hci_init(void)
{
    ...
    hci_pcb->ssp_enable = BT_ENABLE_SSP;    //是否支持安全简单配对
    hci_pcb->ssp_io_cap = BT_SSP_IOCAP_CONF;    //安全简单配对IO能力描述符
    hci_pcb->class_of_device = BT_CLASS_OF_DEVICE;  //设备类型，0x200408
    hci_pcb->local_name = (const uint8_t *)BT_LOCAL_NAME;   //本地设备名称
    hci_pcb->pincode = (uint8_t *)BT_PIN_CODE;  //配对PIN码

    hci_register_pin_req(_hci_pin_req_handle);  //注册PIN码请求回调函数
    /* Clear globals */
...
static err_t _hci_init_cmd_compl_process(uint8_t *payload,uint16_t payload_len)
{
    ...
    if((opcode&HCI_GRP_VENDOR_SPECIFIC) == HCI_GRP_VENDOR_SPECIFIC)
    {
        ...
        if ((hci_pcb->vendor_init_status != VENDOR_INITED) && (hci_pcb->chip_mgr != NULL))
        {
            _hci_vendor_init(HCI_OGF(opcode),HCI_OCF(opcode));
        }
        else
        {
            BT_HCI_TRACE_DEBUG("Vendor already inited, ignore opcode 0x%x\n", opcode);
        }
    }

    if ((status != HCI_SUCCESS) && ((opcode&HCI_GRP_VENDOR_SPECIFIC) != HCI_GRP_VENDOR_SPECIFIC))
    {
        BT_HCI_TRACE_WARNING("Init cmd complete failed: opcode=0x%x status=0x%x\n", opcode, status);
        return BT_ERR_OK;
    }
    ...
    case HCI_OP_RESET:
    {
        ...
        utimer_cancel(hci_pcb->timer);  //取消命令超时定时器,复位作为第一条命令进行保护
        if (hci_pcb->vendor_init_status == VENDOR_NOT_INIT) //进行厂商初始化
        {
            if(hci_pcb->chip_mgr != NULL)   //判断芯片
                _hci_vendor_init(HCI_OGF(opcode),HCI_OCF(opcode));  //传入自己的OPCODE，触发厂商的初始化流程
        }
        else    //厂商初始化完成后触发的第二次RESET命令完成事件
        {
            hci_read_local_version_info();  //读取本地版本信息，触发后续的初始化流程
        }
    ...
    case HCI_OP_READ_LOCAL_VERSION_INFO:
    {
        ...
        hci_read_buffer_size();     //控制流控命令，从HOST到CONTROLLER的流控
        break;
    }
    case HCI_OP_READ_BUFFER_SIZE:
    {
        ...
        hci_read_bd_addr(read_bdaddr_complete); //读取本地BD地址，触发后续的初始化流程
        break;
    }
    case HCI_OP_READ_BD_ADDR:
    {
        ...
        HCI_EVENT_RBD_COMPLETE(hci_pcb, bdaddr); /* Notify application.*/
        hci_write_cod((uint8_t *)&hci_pcb->class_of_device);    //写入本地设备类，触发后续的初始化流程
        break;
    }
    ...
    case HCI_OP_CHANGE_LOCAL_NAME:
    {
        ...
        hci_write_page_timeout(BT_PAGE_TIMEOUT); /* value*0.625ms */        //  寻呼时间设置
        break;
    }
    case HCI_OP_WRITE_PAGE_TOUT:
    {		
        BT_HCI_TRACE_DEBUG("Init recv HCI_OP_WRITE_PAGE_TOUT\n");
		hci_read_local_support_cmd();   //获取支持的HCI命令
...
#if BT_BLE_ENABLE > 0
        hci_set_event_mask(0xffffffff,0x3FFFFFFF);  //设置上报事件掩码
#else
...
#else
        hci_write_current_iac_lap(1, &iac);  //显式设置 GIAC，避免控制器默认值异常
#endif
        break;
    }
    case HCI_OP_WRITE_CURRENT_IAC_LAP:
    {
        BT_HCI_TRACE_DEBUG("Init recv HCI_OP_WRITE_CURRENT_IAC_LAP\n");
        hci_write_scan_enable(HCI_SCAN_EN_INQUIRY | HCI_SCAN_EN_PAGE);  //设置扫描模式
        break;
    }
    case HCI_OP_WRITE_SCAN_ENABLE:
    {
        BT_HCI_TRACE_DEBUG("Init recv HCI_OP_WRITE_SCAN_ENABLE\n");
        if(hci_pcb->init_status == BLUETOOTH_INITING)   //初始化完成，进入工作状态
...
{
    hci_evt_hdr_t *evhdr = p->payload;  //获取头部
    ...
    {
        _hci_init_process(p);   //初始化过程中只处理命令完成事件和厂商事件，其他事件先丢弃
        return;
    }
...
{
    hci_link_t *link;   //连接链路
    static hci_acl_hdr_t *aclhdr;       //acl数据包头部，包含连接句柄和数据长度等信息
    struct bt_pbuf_t *q;

    /* Check if an ACL connection exists */
    link = _hci_get_link_by_addr(bdaddr);   //匹配ACL链路地址
...
#if HCI_FLOW_QUEUEING   //缓存区满时处理，暂存一个数据包
        if(p != NULL)
        {
            /* Packet can be queued? */
            if(link->p != NULL) //已经有一个数据包在等待发送了，丢弃新数据包
            {
    ...
    bt_pbuf_chain(q, p);    //连接头部和数据部分，数据部分由上层协议处理
    ...
    phybusif_output(q, aclhdr->len + q->len,PHYBUSIF_PACKET_TYPE_ACL_DATA); //发送数据包

    --hci_pcb->controller_num_acl;  //缓存递减

    /* Free ACL header. Upper layers will handle rest of packet */
    p = bt_pbuf_dechain(q);     //遵循谁分配谁释放的原则，释放头部
    bt_pbuf_free(q);
    return BT_ERR_OK;
}
...
void hci_acl_input(struct bt_pbuf_t *p) //acl数据接收
{
    uint16_t conhdl;    //链路句柄
    hci_link_t *link = NULL;    //链路结构
	hci_acl_hdr_t *aclhdr = p->payload;  //数据包头部
    ...
    if(hci_pcb->flow)   //控制器到主机的流控，接收一个数据包后，通知控制器可以发送下一个数据包了
    {
    ...
    for(link = hci_active_links; link != NULL; link = link->next)
    {
        if(link->conhdl == conhdl)  //通过12位连接句柄匹配链路
        {
            break;
        }
    }
    ...
    if(link != NULL)
    {
        if(aclhdr->len)
        {
            ...
            l2cap_acl_input(p, &(link->bdaddr));    //将数据包交给上层协议处理
        }
    }
}
...
{
    struct bt_pbuf_t *p;
    if((p = bt_pbuf_alloc(BT_TRANSPORT_TYPE, HCI_RESET_PLEN, BT_PBUF_RAM)) == NULL) //分配内存
    {
        BT_HCI_TRACE_ERROR("ERROR:file[%s],function[%s],line[%d] bt_pbuf_alloc fail\n",__FILE__,__FUNCTION__,__LINE__);
    ...
    /* Assembling command packet */
    p = hci_cmd_ass(p, HCI_RESET, HCI_HOST_C_N_BB, HCI_RESET_PLEN); //组装头部
    hci_pcb->timer = utimer_create(HCI_RESET_TIMEOUT, _hci_reset_cmd_timeout, 0);   //启动定时器
    /* Assembling cmd prameters */
    phybusif_output(p, p->tot_len,PHYBUSIF_PACKET_TYPE_CMD);    //发送整条链路
    bt_pbuf_free(p);
