...
static err_t _l2cap_sig_conn_req_process(l2cap_pcb_t *pcb,struct bt_pbuf_t *p,l2cap_sig_hdr_t *sighdr,struct bd_addr_t *bdaddr)
{
    ...
    uint16_t psm = ((uint16_t *)p->payload)[0];     //PSM
    /* Search for a listening pcb */
    for(lpcb = l2cap_listen_pcbs; lpcb != NULL; lpcb = lpcb->next)  //寻找监听的上层协议
    {
        if(lpcb->psm == psm)
        {
    ...
    if(lpcb == NULL)        //没有找到监听pcb（需要进行注册），拒绝PSM不支持
    {
...
void l2cap_process_sig(struct bt_pbuf_t *q, l2cap_hdr_t *l2caphdr, struct bd_addr_t *bdaddr)
{
    ...
    while(len > 0)
    {
        ...
        bt_pbuf_header(p, -L2CAP_SIGHDR_LEN);   //跳过L2CAP的CODE和ID字段，实现一层一层剥离拿到数据

        /* Check if this is a response/reject signal, and if so, find the matching request */
        if(sighdr->code % 2)   /* if odd this is a resp/rej signal */       //通过奇偶COD判断是请求还是响应/拒绝信号
        {
            BT_L2CAP_TRACE_DEBUG("l2cap_process_sig: Response/reject signal received id = %d code = %d\n",          //收到response以及reject响应命令
                                 sighdr->id, sighdr->code);
            for(pcb = l2cap_active_pcbs; pcb != NULL; pcb = pcb->next)      //遍历寻找匹配的pcb
            {
                for(sig = pcb->unrsp_sigs; sig != NULL; sig = sig->next)
                {
        ...
        if(l2caphdr->len > L2CAP_MTU)       //大于本地可以处理的MTU（不是上层协议可处理的大小是本地信令通道的），回复CommandReject
        {
        ...
        switch(sighdr->code)    //根据CODE字段判断是哪种命令
        {
        case L2CAP_CMD_REJ:     //此处只负责清理资源
            _l2cap_sig_cmd_reject_process(pcb,sig);
            break;
        case L2CAP_CONN_REQ:    //连接请求
...
static l2cap_seg_t * l2cap_reassembly_data(struct bt_pbuf_t *p, struct bd_addr_t *bdaddr,uint8_t *can_continue)
{
    ...
    bt_pbuf_header(p, -HCI_ACL_HDR_LEN);    //跳过HCI ACL头部
    bt_pbuf_realloc(p, aclhdr->len);    //调整bt_pbuf_t的长度为ACL数据部分的长度，padding部分不包含在内(满足传输要求去0x00的padding部分不包含在内)
    //p此时为L2CAP数据部分

    //判断是否有ACL数据分组
    for(inseg = l2cap_insegs; inseg != NULL; inseg = inseg->next)   //对比地址，找到对应的重组分段
    {
    ...
    //收到ACL数据分组，判断是分片还是起始分片
    if(((aclhdr->conhdl_pb_bc >> 12) & 0x03)== L2CAP_ACL_CONT)	 /* Continuing fragment */
    {
        if(inseg == NULL)   //没有START分片，直接丢弃分片
        {
            /* Discard packet */
            BT_L2CAP_TRACE_DEBUG("l2cap_acl_input: Continuing fragment. Discard packet\n");
            bt_pbuf_free(p);
            *can_continue = 0;  //等待下一次分片到来
            return inseg;
        }
        else if(inseg->p->tot_len + p->tot_len > inseg->len)    //分片长度超过L2CAP头部长度，丢弃分片
        {
            /* Check if length of
            							segment exceeds
            							l2cap header length */
    ...
    if(inseg->p->tot_len < inseg->len)  //没收全
    {
...
static err_t _l2cap_classical_sig_cid_process(struct bt_pbuf_t *p,l2cap_hdr_t *l2cap_hdr,struct bd_addr_t *bdaddr)
{
    bt_pbuf_header(p, -L2CAP_HDR_LEN);  //跳过L2CAP头部长度
    l2cap_process_sig(p, l2cap_hdr, bdaddr);    //处理L2CAP信号
    bt_pbuf_free(p);    //处理完信号后释放bt_pbuf_t
...
//ACL数据包接收入口，处理ACL数据包的分段重组，并根据CID将数据包转发到相应的处理函数
void l2cap_acl_input(struct bt_pbuf_t *p, struct bd_addr_t *bdaddr)
{
    uint8_t can_contiue;    //是否继续处理，通过两个参数判断进行处理
    ...
    /* Handle packet */
    switch(inseg->l2caphdr->cid)    //CID进行处理
    {
    case L2CAP_NULL_CID:
        _l2cap_null_cid_process(inseg->p,bdaddr);
        break;
    case L2CAP_SIG_CID:     //L2CAP连接信令控制，直接转发到相应的处理函数
        _l2cap_classical_sig_cid_process(inseg->p,inseg->l2caphdr,bdaddr);
        break;
    case L2CAP_CONNLESS_CID:    //无连接数据，直接丢弃
        _l2cap_connless_cid_process(inseg->p,bdaddr);
        break;
    case L2CAP_ATT_CID:     //BLE属性协议，直接转发到相应的处理函数
        _l2cap_fixed_cid_process(L2CAP_ATT_CID,p,bdaddr);
        break;
	case L2CAP_SM_CID:        //BLE安全管理协议，直接转发到相应的处理函数
...
err_t l2cap_datawrite(l2cap_pcb_t *pcb, struct bt_pbuf_t *p)
{
    ...
    if(pcb->state != L2CAP_OPEN)    //只有在L2CAP连接打开状态才允许发送数据，否则丢弃数据并返回错误码
    {
    ...
    else
    {
        ...
        ret = l2cap_write(&(pcb->remote_bdaddr), q, q->tot_len);    //发送数据给L2CAP层，底层会进行ACL数据包的分段和发送
    }
