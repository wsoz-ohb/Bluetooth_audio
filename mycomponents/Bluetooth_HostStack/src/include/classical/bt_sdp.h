...
/**/
#define SDP_MAX_SRHDLS 12   //最大服务记录句柄
...
/*---SDP相关回调---*/
struct _sdp_pcb_t;
typedef void (* sdp_service_searched_cb)(void *arg, struct _sdp_pcb_t *pcb, uint16_t tot_src, uint16_t curr_src, uint32_t *rhdls);  //服务UUID响应回调(0x02和0x03共用)
typedef void (* sdp_attributes_recv_cb)(void *arg, struct _sdp_pcb_t *pcb, uint16_t attribl_bc,struct bt_pbuf_t *p);  //协议UUID响应回调(0x04和0x05共用)
typedef void (* sdp_attributes_searched_cb)(void *arg, struct _sdp_pcb_t *pcb, uint16_t attribl_bc, struct bt_pbuf_t *p); //服务协议一体回调函数(0x06和0x07共用)
...
typedef struct _sdp_record_t {
  ...
  uint8_t *record_de_list;  // 服务记录的完整内容（Data Element格式）
  uint8_t len;  // record_de_list的字节长度
}sdp_record_t;
...
uint32_t sdp_next_rhdl(void); //生成服务记录句柄的函数32bit

/* Client API */
sdp_pcb_t *sdp_new(l2cap_pcb_t *l2cappcb);  //创建SDP协议控制块
...
err_t sdp_register_service(sdp_record_t *record); //sdp注册，添加服务记录
void sdp_unregister_service(sdp_record_t *record);

/* Lower layer API */
void sdp_lp_disconnected(l2cap_pcb_t *l2cappcb);
err_t sdp_recv(void *arg, l2cap_pcb_t *pcb, struct bt_pbuf_t *p, err_t err);  //sdp接收数据
