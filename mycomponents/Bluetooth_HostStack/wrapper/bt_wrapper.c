...
//    0x75, 0x01,              /*     Report Size (1) */
//    0x95, 0x01,              /*     Report Count (1) */
//    0x81, 0x02,              /*     Input (Data,Var,Abs) - left button */
//
...
{
    uint8_t init_status = BT_INIT_SUCCESS;
...
#if PROFILE_HID_ENABLE > 0
    if(hid_device_init(&hid_wrapper_cb) == BT_ERR_OK &&
       hid_device_sdp_init(hid_device_descriptor,sizeof(hid_device_descriptor)) == BT_ERR_OK)
    {
        bt_profile_mask |= BT_PROFILE_HID_DEVICE_MASK;
    }
    else
    {
        init_status = BT_INIT_FAIL;
        printf("[HID] hid profile init fail\r\n");
    }
#endif
    ...
    {
        bt_wrapper_cb->app_common_cb->bt_init_result(init_status,bt_profile_mask);
    }
