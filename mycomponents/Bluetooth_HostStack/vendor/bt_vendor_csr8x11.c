...
uint8_t csr8x11_initscript[] =
{
    ...
    0x15, 0xc2, 0x02, 0x00, 0x0a, 0x00, 0x0d, 0x00, 0x03, 0x70, 0x00, 0x00, 0xea, 0x01, 0x02, 0x00, 0x08, 0x00,0x0e,0x00,0x00,0x10,
...
void csr8x11_vendor_init(init_done_cb cb,uint8_t ogf,uint8_t ocf)
{
    ...
    uint16_t size = 0;
    uint16_t varid;
    uint16_t script_len = (uint16_t)sizeof(csr8x11_initscript);

    if (prepare_pos >= script_len)
    {
        BT_VENDOR_TRACE_WARNING("csr8x11 init script finished, ignore extra vendor complete\n");
        return;
    }
    ...
    if ((uint32_t)prepare_pos + (uint32_t)size + 1u > script_len)
    {
        BT_VENDOR_TRACE_ERROR("csr8x11 init script overflow: pos=%d size=%d len=%d\n", prepare_pos, size, script_len);
        prepare_pos = script_len;
        return;
    }
